...
bool A3DTerrainWaterRender::LoadWaterResources()
{
 ...
 //	devFmt.nHeight		= pViewParam->Height + 100;
	...
	m_pEdgePixelShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader("Shaders\\ps\\water_edge_ps.txt", false);
	...
	m_pEdgeVertexShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadVertexShader("Shaders\\vs\\water_edge_vs.txt", false, dwVSDecl);
	...
	m_pWaterCheapShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader("Shaders\\ps\\water_cheap.txt", false);
	...
	m_pWaterCheapScrollShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader("Shaders\\ps\\water_cheap_scroll.txt", false);
	...
	m_pWaterExpensiveShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader("Shaders\\ps\\water_expensive.txt", false);
	...
	m_pWaterExpScrollShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader("Shaders\\ps\\water_expensive_scroll.txt", false);
	...
	m_pWaterRefractShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader("Shaders\\ps\\waterrefract.txt", false);
	...
	m_pWaterRefractScrollShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader("Shaders\\ps\\waterrefract_scroll.txt", false);
	if( NULL == m_pWaterRefractShader )
	{
		g_A3DErrLog.Log("A3DTerrainWater::Init(), failed to load waterrefract.txt");
		return false;
	...
	m_pWaterAlphaSimple = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader("Shaders\\ps\\water_alpha_simple.txt", false);
	...
	m_pWaterVertexShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadVertexShader("Shaders\\vs\\water_vs.txt", false, dwVSDecl);
	...
	m_pWaveShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader("Shaders\\ps\\waveblend.txt", false);
	...
	_BuildWaterVertexDecl(1, dwVSDecl);
	m_pWaveVertexShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadVertexShader("Shaders\\vs\\wavevs.txt", false, dwVSDecl);
...
bool A3DTerrainWaterRender::CreateStream(bool bStaticMesh, int nMaxVerts, int nMaxFaces)
{
	...
	if (m_pA3DDevice->TestVertexShaderVersion(1, 1))
	{
		...
		m_pWaterStaticVertexShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadVertexShader("Shaders\\vs\\water_static_mesh_vs.txt", false, dwVSDecl);
...
void A3DTerrainWaterRender::SetWaterType(WATER_TYPE type)
{
	...
	else if (type == WATER_TYPE_GROUND)
	{
		if (!m_pGndReflectShader)
		{
			m_pGndReflectShader = m_pA3DDevice->GetA3DEngine()->GetA3DShaderMan()->LoadPixelShader("Shaders\\ps\\ground_reflect.txt", false);
			if (!m_pGndReflectShader)
			{
