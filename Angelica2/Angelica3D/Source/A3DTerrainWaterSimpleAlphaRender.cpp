...
bool A3DTerrainWaterSimpleAlphaRender::InitWaterAreasHeightRT()
{
	...
	m_pWaterAreasRTVertexShader = pShaderMan->LoadVertexShader("shaders\\vs\\water_areas_rt_vs.txt", false, dwVSDecl);
	if( m_pWaterAreasRTVertexShader == NULL)
		return false;
	
	m_pWaterAreasRTPixelShader = pShaderMan->LoadPixelShader("shaders\\ps\\water_areas_rt_ps.txt", false);
	if( m_pWaterAreasRTPixelShader == NULL)
		return false;
	
	m_pSimpleAlphaEdgeVertexShader = pShaderMan->LoadVertexShader("shaders\\vs\\simple_alpha_edge_vs.txt", false, dwVSDecl);
	if( m_pSimpleAlphaEdgeVertexShader == NULL)
		return false;
	m_pSimpleAlphaEdgePixelShader = pShaderMan->LoadPixelShader("shaders\\ps\\simple_alpha_edge_ps.txt", false);
