...
///////////////////////////////////////////////////////////////////////////
//
//	Implement VERTEXNORMAL
//
///////////////////////////////////////////////////////////////////////////

CMAXMesh::VERTEXNORMAL::VERTEXNORMAL(const Point3& n, DWORD s)
{
	vNormal		= n;
	dwSmooth	= s;
	pNext		= NULL;
...
//	Add a face normal
void CMAXMesh::VERTEXNORMAL::AddNormal(const Point3& n, DWORD s)
{
	if (dwSmooth & s)
	{
		vNormal += n;
	...
	{
		VERTEXNORMAL* pNew = new VERTEXNORMAL(n, s);
		pNext = pNew;
	}
}

//	Normalize
void CMAXMesh::VERTEXNORMAL::Normalize()
{
	VERTEXNORMAL* ptr = pNext;
	VERTEXNORMAL* prev = this;

	while (ptr)
	{ 
		if (ptr->dwSmooth & dwSmooth)
		{
			vNormal += ptr->vNormal;
...
//	m_pMorphR3		= NULL;
	...
	m_tmObject.IdentityMatrix();
	m_tmNode.IdentityMatrix();
	m_tmInitObject.IdentityMatrix();
	m_tmSkinInit.IdentityMatrix();
...
{
	if (m_bDelete) 
		m_pTriObject->DeleteMe();

	int i;

	//	Release all vertex normals
	for (i=0; i < m_aVertNormals.GetSize(); i++)
	{
		VERTEXNORMAL* pNext = m_aVertNormals[i];
		while (pNext)
		{
			VERTEXNORMAL* pCur = pNext;
			pNext = pCur->pNext;
			delete pCur;
		}
	}
...
Point3 CMAXMesh::GetVertexNormal(int iFace, int iCorner, int idVertex, Mesh* pIMesh)
{
	if (!pIMesh && !(pIMesh = GetIMesh()))
		return Point3::Origin;

	//	Check if vertex has specified normal
	MeshNormalSpec* pNormalSpec = pIMesh->GetSpecifiedNormals();
	if (pNormalSpec && pNormalSpec->GetNumFaces())
	{
		int normID = pNormalSpec->Face(iFace).GetNormalID(iCorner);
		if (normID >= 0 && pNormalSpec->Face(iFace).GetSpecified(iCorner))
		{
			//	Ok, use specified normal
			Point3 normal = pNormalSpec->GetNormal(iFace, iCorner);

			if (!IsRigidMesh())
			{
				//	Convert normal to world space at initial binding time
				normal = normal * Transpose(Inverse(m_tmInitObject));
			}
	...
	VERTEXNORMAL* pVertNormal = m_aVertNormals[idVertex];
	if (pVertNormal)
	{
		while (pVertNormal)
		{
			if (pVertNormal->dwSmooth & smGroup)
				return pVertNormal->vNormal;
			
			pVertNormal = pVertNormal->pNext;
		}
	}

	//	Note: m_aVertNormals[idVertex] == NULL means this vertex may be a isolated point.
	//		We just get the normal at current time for isolated vertices, it's may not be correct,
	//		because we should get normal at the time when mesh is bound with bones

	//	Get the "rendered" vertex
	RVertex* pRVertex = pIMesh->getRVertPtr(idVertex);
...
CVertexCandidate* CMAXMesh::GetVertexCandidate(CSkeletonCandidate* pSkeletonCandidate,
											   int iFace, int iCorner, Mesh* pIMesh)
{
	if (!pIMesh && !(pIMesh = GetIMesh()))
	{
	...
	int iVertexId = pIMesh->faces[iFace].v[iCorner];
...
//	Point3 vertex1 = pIMesh->getVert(iVertexId) * m_tmObject;
	...
	Point3 normal = GetVertexNormal(iFace, iCorner, iVertexId, pIMesh);
	...
	*/
		...
		if (pTexMap)
		{
			...
			if (pIMesh->mapSupport(iChannel))
			{
				...
				uvVert = pUVVert[pTVFace[iFace].t[iCorner]];
				bValidUV = true;
			}
			else if (pIMesh->numTVerts > 0)
			{
				uvVert = pIMesh->tVerts[pIMesh->tvFace[iFace].t[iCorner]];
				bValidUV = true;
			}
...
bool CMAXMesh::CreateSkinInitMesh()
{
	...
	if (!m_Initos.obj->CanConvertToType(Class_ID(TRIOBJ_CLASS_ID, 0)))
		return false;
	...
	m_tmSkinInit = InitTM;
	m_tmInitObject = tm * InitTM;

	m_bNormalFlip = true;
	if (DotProd(CrossProd(m_tmInitObject.GetRow(0), m_tmInitObject.GetRow(1)), m_tmInitObject.GetRow(2)) > 0)
		m_bNormalFlip = false;
	...
	if (m_bMorph)
	{
		for (int i=0; i < iNumVert; i++)
		{
			...
			m_aInitVerts[i] = v * m_tmInitObject;

			CalcMorphVertexPosDelta(i, v, m_tmInitObject);
		}
	}
	else
	{
...
bool CMAXMesh::CreateRigidInitMesh(Mesh* pIMesh)
{
	...
	//	Calculate local tm of mesh, for rigid, vertices in m_aInitVerts are
	//	in local space
...
//	Create vertex normals, this function weighting vertex's normals by face angle.
//	See 3DSMAX's SDK help "Computing Face and Vertex Normals" for detail
bool CMAXMesh::CreateVertexNormals()
{
	Mesh* pIMesh = GetIMesh();
	if (!pIMesh)
		return false;
	...
	int i, j;
	int iNumFace = GetFaceCount();
	float aAngles[3];

	AArray<Point3> aFaceNormals;
	aFaceNormals.SetSize(iNumFace, 16);
	AArray<float> aCornerAngles;
	aCornerAngles.SetSize(iNumFace * 3, 16);

	//	First, calculate all face normal
	for (i=0; i < iNumFace; i++)
	{
		int iVertexId = pIMesh->faces[i].v[0];
		Point3 v0 = m_aInitVerts[iVertexId];
		iVertexId = pIMesh->faces[i].v[1];
		Point3 v1 = m_aInitVerts[iVertexId];
		iVertexId = pIMesh->faces[i].v[2];
		Point3 v2 = m_aInitVerts[iVertexId];

		Point3 vEdge1 = v1 - v0;
		Point3 vEdge2 = v2 - v0;
		Point3 vNormal = vEdge1 ^ vEdge2;
		aFaceNormals[i] = Normalize(vNormal);

		//	Need to flip normal ?
		if (m_bNormalFlip)
			aFaceNormals[i] = -aFaceNormals[i];

		//	Calculate angle of face's 3 angles
		CalcFaceAngles(v0, v1, v2, aAngles);
		int iIndex = i * 3;
		aCornerAngles[iIndex++] = aAngles[0];
		aCornerAngles[iIndex++] = aAngles[1];
		aCornerAngles[iIndex++] = aAngles[2];
	}

	m_aVertNormals.SetSize(m_iNumVert, 16);
	memset(m_aVertNormals.GetData(), 0, sizeof (VERTEXNORMAL*) * m_iNumVert);

	//	Accumulate vertex normals considering face's smoothing group
	for (i=0; i < iNumFace; i++)
	{
		//	Get the smoothing group of the face
		Face* pFace = &pIMesh->faces[i];
		DWORD smGroup = pFace->smGroup;
		const Point3& vFaceNormal = aFaceNormals[i];
		int iBaseIdx = i * 3;

		for (j=0; j < 3; j++)
		{
			int iVertexId = pIMesh->faces[i].v[j];
			ASSERT(iVertexId >= 0 && iVertexId < m_iNumVert);

			//	Multiply face normal and corner weight
			float fAngleWeight = aCornerAngles[iBaseIdx + j];
			Point3 vNormal = vFaceNormal * fAngleWeight;

			if (!m_aVertNormals[iVertexId])
			{
				m_aVertNormals[iVertexId] = new VERTEXNORMAL(vNormal, smGroup);
			}
			else
			{
				m_aVertNormals[iVertexId]->AddNormal(vNormal, smGroup);
			}
		}
	}
...
bool CMAXMesh::CalcFaceAngles(const Point3& v0, const Point3& v1, const Point3& v2, float* aAngles)
{
	Point3 vEdge1 = v1 - v0;
	Point3 vEdge2 = v2 - v1;
	Point3 vEdge3 = v0 - v2;

	float fLen1 = Length(vEdge1);
	float fLen2 = Length(vEdge2);
	float fLen3 = Length(vEdge3);

	//	If edge length = 0, corner angle should use 0 instead 90.
	if (fLen1) vEdge1 = vEdge1 / fLen1;
	if (fLen2) vEdge2 = vEdge2 / fLen2;
	if (fLen3) vEdge3 = vEdge3 / fLen3;

	if (fLen1 && fLen3)
	{
		float fDot = DotProd(-vEdge3, vEdge1);
		a_Clamp(fDot, -1.0f, 1.0f);
		aAngles[0] = acos(fDot);
	}
	else
		aAngles[0] = 0.0f;
	...
	if (fLen1 && fLen2)
	{
		float fDot = DotProd(-vEdge1, vEdge2);
		a_Clamp(fDot, -1.0f, 1.0f);
		aAngles[1] = acos(fDot);
	}
	else
	...
	if (fLen2 && fLen3)
	{
		float fDot = DotProd(-vEdge2, vEdge3);
		a_Clamp(fDot, -1.0f, 1.0f);
		aAngles[2] = acos(fDot);
	}
	else
