...
*/
int WindingOnPlaneSide(s_WINDING* pWinding, A3DVECTOR3& vNormal, FLOAT fDist)
{
	...
	for (int i=0; i < pWinding->iNumPts; i++)
	{
		if ((iSide = PointOnPlaneSide(pWinding->pVerts[i], vNormal, fDist)) == SIDE_FRONT)
		{
			if (bBack)
				return SIDE_CROSS;
