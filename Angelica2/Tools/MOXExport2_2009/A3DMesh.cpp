...
bool A3DMesh::Init(IDirect3DDevice8 * pDevice, int nFrameCount, int nVertCount, int nIndexCount, bool bWire, bool bSupportLightMap)
{
	int i;
...
bool A3DMesh::SetVerts(int nFrame, A3DVERTEX * pVerts, int nVertCount, A3DLIGHTMAPCOORD* pLightMapCoord)
{
	...
	if( pLightMapCoord != NULL)
	{
		memcpy(m_ppLightMapCoord[nFrame], pLightMapCoord, sizeof(A3DLIGHTMAPCOORD)* m_nVertCount);
	}
...
bool A3DMesh::AddMoreVertex(A3DVERTEX * pVerts, int nVertCount,	WORD * pIndices, int nIndexCount, A3DLIGHTMAPCOORD* pLightMapCoord)
{
	int i;
	...
	//Update Light map coord
	if( pLightMapCoord != NULL)
	{
		m_ppLightMapCoord[0] = (A3DLIGHTMAPCOORD*) realloc(m_ppLightMapCoord[0], sizeof(A3DLIGHTMAPCOORD)* (m_nVertCount + nVertCount));
		if( NULL == m_ppLightMapCoord[0]) return false;
		memcpy(&m_ppLightMapCoord[0][m_nVertCount], pLightMapCoord, sizeof(A3DLIGHTMAPCOORD) * nVertCount);
	}
...
bool A3DMesh::CleanMesh()
{
	...
	for(i=0; i<nNewVertCount; i++)
	{
		for(n=0; n<m_nFrameCount; n++)
		{
			...
			if( m_bSupportLightMap)
			{
				m_ppLightMapCoord[n][i] = m_ppLightMapCoord[n][pInvMapTab[i]];
			}
		}
	}			  
